...
import { 
  Search, Bell, User, Loader2, Plus, Edit, Eye, Filter, Lock, Unlock, Stethoscope, Clock
} from 'lucide-react';
import dayjs from 'dayjs';

import Sidebar from '@/app/admin/Sidebar';
import { getDoctors, getDoctorSpecialties, updateDoctorStatus } from '@/app/admin/doctors/actions';
...
export default function AdminDoctorsPage() {
  ...
  const fetchData = async () => {
    ...
    if (docsRes.success && docsRes.data) {
      ...
      if (docsRes.message === 'Không có quyền truy cập') {
        router.push('/login');
      }
    }
  ...
  const toggleStatus = async (id: number, currentStatus: string) => {
    ...
    if (!confirm(`Bạn có chắc chắn muốn ${newStatus === 'Khóa' ? 'khóa' : 'mở khóa'} bác sĩ này?`)) return;
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0 z-10 shadow-sm">
          <div className="flex items-center gap-6">
            <h1 className="text-xl font-bold text-slate-900 flex items-center gap-2">
              <Stethoscope className="text-blue-600"/> Quản lý Bác sĩ
            </h1>
          </div>
          <div className="flex items-center gap-6">
        ...
        <div className="flex-1 overflow-y-auto p-8 animate-in fade-in duration-500">
          ...
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 mb-8 flex flex-col lg:flex-row items-center gap-4 justify-between">
            ...
            <Link href="/admin/doctors/create" className="bg-blue-600 text-white px-6 py-2.5 rounded-xl font-bold hover:bg-blue-700 shadow-md transition flex items-center gap-2 text-sm">
                ...
                <table className="w-full text-left text-sm whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {doctors.length > 0 ? doctors.map((doc: any) => {
                      ...
                      const status = profile?.status || 'Đang làm việc';
                      
                      let statusBadge = '';
                      if (status === 'Hoạt động' || status === 'Đang làm việc') {
                      ...
                      return (
                        <tr key={doc.id} className="hover:bg-slate-50 transition-colors">
                          <td className="px-6 py-4 font-bold text-slate-900">{profile?.doctorCode || 'N/A'}</td>
                          <td className="px-6 py-4 font-medium text-slate-900 flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold text-xs">
                              {profile?.imagePrefix || 'BS'}
                            </div>
                            {doc.fullName}
                          </td>
                          <td className="px-6 py-4 text-slate-600">{profile?.specialty}</td>
                          <td className="px-6 py-4 text-slate-600">{doc.phone}</td>
                          <td className="px-6 py-4">
                            <span className={`px-3 py-1 text-xs font-bold rounded-lg flex items-center gap-1.5 w-max ${statusBadge}`}>
                              ...
                              {status === 'Đang làm việc' ? 'Hoạt động' : status}
                            </span>
                          </td>
                          <td className="px-6 py-4 text-right">
                            <div className="flex items-center justify-end gap-2">
                              <Link href={`/admin/doctors/${doc.id}`} className="p-2 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition" title="Xem chi tiết">
                                <Eye size={16}/>
                              </Link>
                              <Link href={`/admin/doctors/${doc.id}?edit=true`} className="p-2 text-orange-600 bg-orange-50 hover:bg-orange-100 rounded-lg transition" title="Sửa">
                                <Edit size={16}/>
                              </Link>
                              {status === 'Khóa' ? (
                                <button onClick={() => toggleStatus(doc.id, status)} className="p-2 text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-lg transition" title="Mở khóa">
                                  <Unlock size={16}/>
                                </button>
                              ) : (
                                <button onClick={() => toggleStatus(doc.id, status)} className="p-2 text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition" title="Khóa tài khoản">
                                  <Lock size={16}/>
                                </button>
                              )}
                            </div>
                      ...
                      <tr><td colSpan={6} className="px-6 py-12 text-center text-slate-500">Không tìm thấy bác sĩ nào phù hợp.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
