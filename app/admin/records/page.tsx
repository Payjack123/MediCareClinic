...
import { 
  Search, Bell, Clock, Eye, Filter, FileText, CalendarDays
} from 'lucide-react';
import dayjs from 'dayjs';
import toast from 'react-hot-toast';

import Sidebar from '@/app/admin/Sidebar';
import { getExaminations } from '@/app/admin/records/actions';
...
export default function AdminRecordsPage() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    const res = await getExaminations(filters);
        ...
        <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0 z-10 shadow-sm">
          <div className="flex items-center gap-6">
            <h1 className="text-xl font-bold text-slate-900 flex items-center gap-2">
              <FileText className="text-blue-600"/> Quản lý Hồ sơ Bệnh án
            </h1>
          </div>
          <div className="flex items-center gap-6">
                ...
                <table className="w-full text-left text-sm whitespace-nowrap">
                  <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 font-bold uppercase text-xs tracking-wider">
                    <tr>
                      ...
                      <th className="px-6 py-4">Bác sĩ khám</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {records.length > 0 ? records.map((rec: any) => (
                      <tr key={rec.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-6 py-4 font-bold text-slate-900">{rec.appointment?.appointmentCode || `HS-${rec.id}`}</td>
                        <td className="px-6 py-4 font-medium text-slate-900">
                          {rec.patient?.fullName} <br/>
                          <span className="text-xs text-slate-500 font-normal">{rec.patient?.phone} | {rec.patient?.patientProfile?.patientCode}</span>
                        </td>
                        <td className="px-6 py-4 font-medium text-slate-900">
                          BS. {rec.doctor?.fullName}
                        </td>
                        <td className="px-6 py-4 text-slate-700">{rec.appointment?.specialty || 'N/A'}</td>
                        <td className="px-6 py-4 text-slate-700 font-medium">{dayjs(rec.createdAt).format('DD/MM/YYYY')}</td>
                        <td className="px-6 py-4 text-slate-700 max-w-[200px] truncate" title={rec.diagnosis}>
                          {rec.diagnosis || 'Chưa cập nhật'}
                        </td>
                        <td className="px-6 py-4">
                          <span className="px-3 py-1 text-xs font-bold rounded-lg flex items-center gap-1.5 w-max bg-emerald-100 text-emerald-700">
                            Hoàn thành
                          </span>
                        </td>
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <Link href={`/admin/records/${rec.id}`} className="px-3 py-2 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition text-xs font-bold flex items-center gap-1" title="Xem chi tiết">
                      ...
                      <tr><td colSpan={8} className="px-6 py-12 text-center text-slate-500">Không tìm thấy hồ sơ nào phù hợp.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
